...
#if UNITY_EDITOR
	private Dictionary<Texture2D,TextureImporterFormat> _textureFormatMap = new Dictionary<Texture2D, TextureImporterFormat>();
#endif
...
#if UNITY_EDITOR		
	void setTextureFormat(Texture2D tx, TextureImporterFormat texFormat, bool addToList){		
		AssetImporter ai = AssetImporter.GetAtPath( AssetDatabase.GetAssetOrScenePath(tx) );
		if (ai != null && ai is TextureImporter){
			TextureImporter textureImporter = (TextureImporter) ai;
			if (textureImporter.textureFormat != texFormat){
				Debug.Log("Setting format of texture " + tx + " to " + texFormat);
				if (addToList) _textureFormatMap.Add(tx,textureImporter.textureFormat);
				textureImporter.textureFormat = texFormat;	
				AssetDatabase.ImportAsset(AssetDatabase.GetAssetOrScenePath(tx));
			}
		}
	}
