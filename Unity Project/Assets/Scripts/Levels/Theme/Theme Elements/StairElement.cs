...
public class StairElement : ThemeElement
{
    ...
    {
        List<Bounding> options = obj.Grids.FindRectangles(GridWidth, GridHeight, true, new StrokedAction<GenSpace>()
            {
                UnitAction = unitTest,
        ...
        {
            if (obj.Grids.DrawEdge(placed, loc, unitTest, false))
            {
                side = loc;
                break;
            }
        }
        obj.Grids.DrawEdge(placed, side, Draw.SetTo(GridType.StairPlace, theme));
        return true;
    }
}
