    ...
    , IRaycastableGraph
    {
        ...
        public override void Scan()
        {
            ...
            foreach (GridSpace val in level.GetEnumerateValues())
            {
                ...
                textureData.Apply(node, xz.x, xz.z);
        ...
        private Int3 ConvertToGrid(Int3 pos)
        {
            Vector3 vpos = inverseMatrix.MultiplyPoint3x4((Vector3)pos);
            vpos.x -= 0.5F;
            vpos.z -= 0.5F;
            return new Int3(Mathf.RoundToInt(vpos.x), Mathf.RoundToInt(vpos.y), Mathf.RoundToInt(vpos.z));
        }
        ...
        public override void OnDrawGizmos(bool drawNodes)
        {
            ...
            for (int z = 0; z < depth; z++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    if (!node.walkable)
                    {// || node.activePath != AstarPath.active.debugPath)  
                        continue;
                    }
